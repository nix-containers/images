#!/usr/bin/env bash
# Update script for kube-rbac-proxy package
...
if [[ $# -ge 1 ]]; then
    ...
    log_info "Fetching latest kube-rbac-proxy version from GitHub..."
...
VENDOR_HASH=$(get_vendor_hash "kube-rbac-proxy" "$USE_LIMA")
...
if [[ "$USE_LIMA" == "true" ]]; then
    BUILD_OUTPUT=$(limactl shell default -- bash -c ". /nix/var/nix/profiles/default/etc/profile.d/nix-daemon.sh && nix build .#kube-rbac-proxy --print-build-logs -L" 2>&1 || true)
else
    BUILD_OUTPUT=$(nix build .#kube-rbac-proxy --print-build-logs -L 2>&1 || true)
fi
...
log_info "Successfully updated kube-rbac-proxy to version $VERSION"
echo ""
echo "Updated files:"
echo "  - $DEFAULT_NIX"
