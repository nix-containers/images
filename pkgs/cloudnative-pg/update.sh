#!/usr/bin/env bash
# Update script for cloudnative-pg package
...
if [[ $# -ge 1 ]]; then
    ...
    log_info "Fetching latest cloudnative-pg version from GitHub..."
...
VENDOR_HASH=$(get_vendor_hash "cloudnative-pg" "$USE_LIMA")
...
if [[ "$USE_LIMA" == "true" ]]; then
    BUILD_OUTPUT=$(limactl shell default -- bash -c ". /nix/var/nix/profiles/default/etc/profile.d/nix-daemon.sh && nix build .#cloudnative-pg --print-build-logs -L" 2>&1 || true)
else
    BUILD_OUTPUT=$(nix build .#cloudnative-pg --print-build-logs -L 2>&1 || true)
fi
...
log_info "Successfully updated cloudnative-pg to version $VERSION"
echo ""
echo "Updated files:"
echo "  - $DEFAULT_NIX"
