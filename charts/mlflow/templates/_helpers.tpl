...
- name: copy-certs
  ...
  command:
    ...
    - |
      #!/bin/bash
      cp -L /tmp/certs/* /nix-containers/mlflow/certs
  ...
  volumeMounts:
    ...
    - name: mlflow-certificates
      mountPath: /nix-containers/mlflow/certs
...
- name: get-default-auth-conf
  ...
  command:
    ...
    - |
      #!/bin/bash
      cp /nix-containers/mlflow-basic-auth/basic_auth.ini /nix-containers/rendered-basic-auth/basic_auth.ini
  volumeMounts:
    - name: tmp
      mountPath: /tmp
    - name: rendered-basic-auth
      mountPath: /nix-containers/rendered-basic-auth
  {{- if .Values.tracking.resources }}
  resources: {{- toYaml .Values.tracking.resources | nindent 4 }}
  {{- else if ne .Values.tracking.resourcesPreset "none" }}
  resources: {{- include "common.resources.preset" (dict "type" .Values.tracking.resourcesPreset) | nindent 4 }}
...
- name: render-auth-conf
  ...
  command:
    ...
    - |
      ...
      render-template /nix-containers/basic-auth-overrides/*.ini > /tmp/rendered-overrides.ini
      ...
      do
        ...
        ini-file set -s mlflow -k "$key" -v "$value" /nix-containers/rendered-basic-auth/basic_auth.ini
  ...
  volumeMounts:
    ...
    - name: basic-auth-overrides
      mountPath: /nix-containers/basic-auth-overrides
    - name: rendered-basic-auth
      mountPath: /nix-containers/rendered-basic-auth
  {{- if .Values.tracking.resources }}
  resources: {{- toYaml .Values.tracking.resources | nindent 4 }}
...
{{- if .Values.tracking.tls.autoGenerated }}
    {{- printf "/nix-containers/mlflow/certs/tls.crt" -}}
{{- else -}}
    {{- required "Certificate filename is required when TLS in enabled" .Values.tracking.tls.certFilename | printf "/nix-containers/mlflow/certs/%s" -}}
{{- end -}}
{{- end -}}
...
{{- if .Values.tracking.tls.autoGenerated }}
    {{- printf "/nix-containers/mlflow/certs/tls.key" -}}
{{- else -}}
{{- required "Certificate Key filename is required when TLS in enabled" .Values.tracking.tls.certKeyFilename | printf "/nix-containers/mlflow/certs/%s" -}}
{{- end -}}
{{- end -}}
...
{{- if .Values.tracking.tls.autoGenerated }}
    {{- printf "/nix-containers/mlflow/certs/ca.crt" -}}
{{- else -}}
    {{- if .Values.tracking.tls.certCAFilename -}}
    {{- printf "/nix-containers/mlflow/certs/%s" .Values.tracking.tls.certCAFilename -}}
    {{- end -}}
{{- end -}}
{{- end -}}
...
- name: git-clone-repository
  ...
  args:
    ...
    - |
      #!/bin/bash
      rm -rf /app/*
      [[ -f "/opt/nix-containers/scripts/git/entrypoint.sh" ]] && source "/opt/nix-containers/scripts/git/entrypoint.sh"
