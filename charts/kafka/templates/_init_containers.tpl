...
- name: prepare-config
  ...
  args:
    - -ec
    - |
      . /opt/nix-containers/scripts/libkafka.sh
      ...
      [[ -f "/nix-containers/kafka/data/meta.properties" ]] && ID="$(grep "node.id" /nix-containers/kafka/data/meta.properties | awk -F '=' '{print $2}')"
      ...
      if [[ "controller" =~ "$POD_ROLE" ]]; then
          ...
          for ((i = 0; i < {{ int .context.Values.controller.replicaCount }}; i++)); do
              ...
              [[ $i -eq $POD_ID ]] && [[ -f "/nix-containers/kafka/data/meta.properties" ]] && DIR_ID="$(grep "directory.id" /nix-containers/kafka/data/meta.properties | awk -F '=' '{print $2}')"
  ...
  volumeMounts:
    - name: data
      mountPath: /nix-containers/kafka
    - name: kafka-config
      mountPath: /config
    ...
    - name: kafka-sasl
      mountPath: /opt/nix-containers/kafka/config/secrets
      readOnly: true
    {{- end }}
{{- end -}}
