...
# nix-containers Secure Images Helm chart for ExternalDNS
...
```console
helm install my-release oci://registry-1.docker.io/nix-containerscharts/external-dns
```
...
This chart bootstraps a [ExternalDNS](https://github.com/nix-containers/containers/tree/main/nix-containers/external-dns) deployment on a [Kubernetes](https://kubernetes.io) cluster using the [Helm](https://helm.sh) package manager.
...
With the ExternalDNS chart, you can deploy Webhook Providers and configure ExternalDNS easely using `sidecars` and `extraArgs` values:
...
Some of the chart values were changed to adapt to the latest nix-containers standards. More specifically:
...
- Default image changes from `registry.opensource.zalan.do/teapot/external-dns` to `nix-containers/external-dns`.
- The parameters below are renamed:
