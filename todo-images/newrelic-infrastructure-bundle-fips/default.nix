{ buildCLIImage, pkgs, lib, ... }:

# Image: newrelic-infrastructure-bundle-fips
# Reference: https://images.chainguard.dev/directory/image/newrelic-infrastructure-bundle-fips/overview

# Packages available in nixpkgs:
#   pkgs.alsa-lib  # alsa-lib (1.2.14-r2)
#   pkgs.freetype  # freetype (2.14.1-r0)
#   pkgs.giflib  # giflib (5.2.2-r11)
#   pkgs.glibc  # glibc (2.42-r4)
#   pkgs.kmod  # kmod (34.2-r42)
#   pkgs.lcms2  # lcms2 (2.17-r5)
#   pkgs.libffi  # libffi (3.5.2-r1)
#   pkgs.libgcc  # libgcc (15.2.0-r6)
#   pkgs.libjpeg_turbo  # libjpeg-turbo (3.1.2-r1)
#   pkgs.libpng  # libpng (1.6.52-r0)
#   pkgs.libtasn1  # libtasn1 (4.20.0-r5)
#   pkgs.libx11  # libx11 (1.8.12-r3)
#   pkgs.libxau  # libxau (1.0.12-r3)
#   pkgs.libxcb  # libxcb (1.17.0-r8)
#   pkgs.libxdmcp  # libxdmcp (1.1.5-r9)
#   pkgs.libxext  # libxext (1.3.6-r7)
#   pkgs.libxi  # libxi (1.8.2-r4)
#   pkgs.libxrender  # libxrender (0.9.12-r4)
#   pkgs.libxtst  # libxtst (1.2.5-r4)
#   pkgs.p11-kit  # p11-kit (0.25.10-r0)
#   pkgs.tini  # tini (0.19.0-r22)
#   pkgs.xz  # xz (5.8.1-r6)
#   pkgs.zlib  # zlib (1.3.1-r51)

# Packages NOT in nixpkgs (need custom derivations):
#   bcfips-policy-140-3 (1-r26)
#   bouncycastle-fips (2.1.1-r3)
#   bouncycastle-mail-fips (2.1.6-r2)
#   bouncycastle-pg-fips (2.1.11-r2)
#   bouncycastle-pkix-fips (2.1.9-r2)
#   bouncycastle-rng-jent (1.3.6-r1)
#   bouncycastle-tls-fips (2.1.20-r2)
#   bouncycastle-util-fips (2.1.4-r2)
#   ca-certificates (20251003-r0)
#   fontconfig-config (2.17.1-r1)
#   java-cacerts (20251003-r0)
#   ld-linux (2.42-r4)
#   libbrotlicommon1 (1.2.0-r1)
#   libbrotlidec1 (1.2.0-r1)
#   libbz2-1 (1.0.8-r21)
#   libcrypto3 (3.6.0-r4)
#   libexpat1 (2.7.3-r0)
#   libfontconfig1 (2.17.1-r1)
#   libzstd1 (1.5.7-r5)
#   newrelic-infrastructure-agent-fips (1.71.1-r0)
#   newrelic-infrastructure-bundle-fips (3.3.6-r0)
#   nri-apache-fips (1.15.3-r1)
#   nri-cassandra-fips (2.16.0-r1)
#   nri-consul-fips (2.9.4-r1)
#   nri-couchbase-fips (2.8.3-r1)
#   nri-discovery-kubernetes-fips (1.13.6-r0)
#   nri-elasticsearch-fips (5.4.5-r1)
#   nri-f5-fips (2.9.1-r1)
#   nri-haproxy-fips (3.2.3-r1)
#   nri-jmx-fips (3.11.1-r1)
#   nri-kafka-fips (3.15.0-r2)
#   nri-memcached-fips (2.7.3-r1)
#   nri-mongodb-fips (2.10.0-r1)
#   nri-mssql-fips (2.25.1-r1)
#   nri-mysql-fips (1.18.1-r1)
#   nri-nagios-fips (2.11.3-r1)
#   nri-nginx-fips (3.6.3-r1)
#   nri-postgresql-fips (2.22.1-r1)
#   nri-rabbitmq-fips (2.15.3-r1)
#   nri-redis-fips (1.12.6-r1)
#   nrjmx (2.10.2-r0)
#   openjdk-11-jre (11.0.29-r1)
#   openjdk-17-jre (17.0.17-r2)
#   openjdk-bcfips-policy-140-3-j11 (1-r26)
#   openjdk-bcfips-policy-140-3-j17 (1-r26)
#   openssl-config-fipshardened (3.6.0-r4)
#   openssl-fips-test (0.6-r0)
#   openssl-provider-fips-3.1.2 (3.1.2-r3)
#   p11-kit-trust (0.25.10-r0)
#   ttf-dejavu (2.37-r7)

# TODO: Implement newrelic-infrastructure-bundle-fips image
throw "Image 'newrelic-infrastructure-bundle-fips' is not yet implemented"

# Suggested implementation:
# buildCLIImage {
#   drv = pkgs.alsa-lib;
#   name = "newrelic-infrastructure-bundle-fips";
#   tag = "v${pkgs.alsa-lib.version}";
#   entrypoint = [ "${pkgs.alsa-lib}/bin/BINARY" ];
#   cmd = [ "--help" ];
#   user = "0";
#
#   labels = {
#     "org.opencontainers.image.title" = "newrelic-infrastructure-bundle-fips";
#     "org.opencontainers.image.description" = "TODO: Add description";
#   };
# }
