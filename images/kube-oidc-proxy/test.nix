...
pkgs.writeShellScript "test-kube-oidc-proxy" ''
  set -euo pipefail
  echo "Testing kube-oidc-proxy image..."
  ...
  echo "  Checking oidc-proxy runs..."
  out=$(docker run --rm ${image.imageName}:test --version 2>&1 || true)
  if [ -z "$out" ]; then
    out=$(docker run --rm --entrypoint oidc-proxy ${image.imageName}:test --help 2>&1 || true)
  fi
  [ -n "$out" ]
