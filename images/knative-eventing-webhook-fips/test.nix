...
pkgs.writeShellScript "test-knative-eventing-webhook-fips" ''
  set -euo pipefail
  echo "Testing knative-eventing-webhook-fips image..."
  ...
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c \
    'command -v knative-eventing-webhook >/dev/null 2>&1 || ls /nix/store/*/bin/knative-eventing-webhook >/dev/null 2>&1' || true
  ...
  echo "All knative-eventing-webhook-fips tests passed!"
''
