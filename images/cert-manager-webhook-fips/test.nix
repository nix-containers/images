...
pkgs.writeShellScript "test-cert-manager-webhook-fips" ''
  ...
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c 'command -v cert-manager-webhook >/dev/null 2>&1 || ls /nix/store/*/bin/cert-manager-webhook >/dev/null 2>&1' || true
