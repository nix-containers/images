...
pkgs.writeShellScript "test-nri-nginx-fips" ''
  set -euo pipefail
  echo "Testing nri-nginx-fips image..."
  ...
  echo "  Checking nginx binary is present..."
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c 'ls /nix/store/*/bin/nginx >/dev/null 2>&1 && echo ok' | grep -q ok

  # 2. Image shell smoke test
  echo "  Checking image shell works..."
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c 'ls -la / >/dev/null && echo ok' | grep -q ok

  echo "All nri-nginx-fips tests passed!"
''
