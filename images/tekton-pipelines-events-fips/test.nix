...
pkgs.writeShellScript "test-tekton-pipelines-events-fips" ''
  set -euo pipefail
  echo "Testing tekton-pipelines-events-fips image..."
  ...
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c 'command -v pipelines-events >/dev/null 2>&1 || ls /nix/store/*/bin/pipelines-events >/dev/null 2>&1' || true

  echo "All tekton-pipelines-events-fips tests passed!"
''
