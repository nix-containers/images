...
pkgs.writeShellScript "test-envoy-gateway" ''
  ...
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c 'command -v envoy-gateway >/dev/null 2>&1 || ls /usr/bin/envoy-gateway >/dev/null 2>&1 || ls /nix/store/*/bin/envoy-gateway >/dev/null 2>&1' || true
