...
pkgs.writeShellScript "test-jaeger-remote-storage-fips" ''
  set -euo pipefail
  echo "Testing jaeger-remote-storage-fips image..."

  # Smoke test: the image starts a shell and the expected binary is present.
  # (jaeger-remote-storage-fips is a long-running server; we avoid guessing version/help flags and
  ...
  echo "All jaeger-remote-storage-fips tests passed!"
''
