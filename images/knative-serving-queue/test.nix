...
pkgs.writeShellScript "test-knative-serving-queue" ''
  set -euo pipefail
  echo "Testing knative-serving-queue image..."
  ...
  echo "  Checking knative-serving-queue binary is present..."
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c \
    'command -v knative-serving-queue >/dev/null 2>&1 || ls /nix/store/*/bin/knative-serving-queue >/dev/null 2>&1' || true
  ...
  echo "All knative-serving-queue tests passed!"
''
