#!/usr/bin/env bash
# timescaledb-compat docker-entrypoint.sh
#
# PostgreSQL + TimescaleDB. Copied from the postgres image's entrypoint with one
# addition: on first boot it appends `shared_preload_libraries = 'timescaledb'`
# to postgresql.conf (required before `CREATE EXTENSION timescaledb`).
#
...
initialize_db() {
  ...
  # timescaledb must be preloaded for `CREATE EXTENSION timescaledb` to succeed.
  # Persist it to postgresql.conf so both the temp instance started just below
  # AND the final server load it. Disable telemetry (isolated container, no
  # outbound needed).
  {
