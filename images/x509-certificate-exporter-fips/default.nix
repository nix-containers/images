...
# x509-certificate-exporter-fips — built from the upstream release artifact (#618).
let
  version = "4.1.0";
  ...
  drv = pkgs.runCommand "x509-certificate-exporter-fips-4.1.0" { nativeBuildInputs = [ pkgs.gnutar pkgs.gzip pkgs.unzip ]; } ''
...
mkImage {
  ...
  labels = {
    ...
    "org.opencontainers.image.description" = "x509-certificate-exporter-fips (built from upstream release 4.1.0)";
  };
}
