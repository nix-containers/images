...
pkgs.writeShellScript "test-kubeflow-pipelines-persistence_agent" ''
  ...
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c 'command -v pipelines-persistence_agent >/dev/null 2>&1 || ls /nix/store/*/bin/pipelines-persistence_agent >/dev/null 2>&1'
