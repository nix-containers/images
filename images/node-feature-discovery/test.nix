...
pkgs.writeShellScript "test-node-feature-discovery" ''
  ...
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c 'command -v node-feature-discovery >/dev/null 2>&1 || ls /nix/store/*/bin/node-feature-discovery >/dev/null 2>&1' || true
