...
pkgs.writeShellScript "test-jaeger-query" ''
  set -euo pipefail
  echo "Testing jaeger-query image..."

  # Smoke test: the image starts a shell and the expected binary is present.
  # (jaeger-query is a long-running server; we avoid guessing version/help flags and
  ...
  echo "All jaeger-query tests passed!"
''
