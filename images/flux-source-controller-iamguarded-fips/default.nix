...
let
  ...
  component = buildGoModule {
    pname = "flux-source-controller-iamguarded-fips";
    inherit version;
    src = fetchFromGitHub {
      owner = "fluxcd";
      repo = "flux2";
...
in mkImage {
  ...
  tag = "v${version}";
  entrypoint = [ "${component}/bin/flux-source-controller-iamguarded" ];
  cmd = [];
  extraPkgs = with pkgs; [ cacert git ];
  labels = {
    "org.opencontainers.image.title" = "flux-source-controller-iamguarded-fips";
    "org.opencontainers.image.description" = "Flux flux-source-controller-iamguarded";
