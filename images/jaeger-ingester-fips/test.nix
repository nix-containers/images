...
pkgs.writeShellScript "test-jaeger-ingester-fips" ''
  set -euo pipefail
  echo "Testing jaeger-ingester-fips image..."

  # Smoke test: the image starts a shell and the expected binary is present.
  # (jaeger-ingester-fips is a long-running server; we avoid guessing version/help flags and
  ...
  echo "All jaeger-ingester-fips tests passed!"
''
