...
pkgs.writeShellScript "test-kube-proxy-fips-1.34-default" ''
  set -euo pipefail
  echo "Testing kube-proxy-fips-1.34-default image..."
  ...
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c \
    'command -v proxy >/dev/null 2>&1 || ls /nix/store/*/bin/proxy >/dev/null 2>&1' || true

  echo "All kube-proxy-fips-1.34-default tests passed!"
''
