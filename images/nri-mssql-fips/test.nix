...
pkgs.writeShellScript "test-nri-mssql-fips" ''
  set -euo pipefail
  echo "Testing nri-mssql-fips image..."
  ...
  echo "  Checking mssql binary is present..."
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c 'ls /nix/store/*/bin/mssql >/dev/null 2>&1 && echo ok' | grep -q ok

  # 2. Image shell smoke test
  echo "  Checking image shell works..."
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c 'ls -la / >/dev/null && echo ok' | grep -q ok

  echo "All nri-mssql-fips tests passed!"
''
