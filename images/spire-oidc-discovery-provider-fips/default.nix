...
# spire-oidc-discovery-provider-fips — built from the upstream release artifact (#618).
let
  version = "1.15.2";
  ...
  drv = pkgs.runCommand "spire-oidc-discovery-provider-fips-1.6.2" { nativeBuildInputs = [ pkgs.gnutar pkgs.gzip pkgs.unzip ]; } ''
...
mkImage {
  ...
  labels = {
    ...
    "org.opencontainers.image.description" = "spire-oidc-discovery-provider-fips (built from upstream release 1.6.2)";
  };
}
