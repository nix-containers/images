...
# confluent-kafka
# =============
# Apache Kafka broker. Confluent's cp-kafka is built on the Apache Kafka core;
# this packages that core (Confluent's proprietary add-ons are not included).
# https://kafka.apache.org/  https://www.confluent.io/
#
...
# sibling `kafka` image: a single-node KRaft broker+controller. Operators mount
# their own config for a real cluster.
...
mkImage {
  ...
  labels = {
    ...
    "org.opencontainers.image.description" = "Apache Kafka broker (the core that Confluent cp-kafka builds on) - distributed event streaming platform";
