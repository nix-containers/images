...
pkgs.writeShellScript "test-tekton-pipelines-webhook-fips" ''
  set -euo pipefail
  echo "Testing tekton-pipelines-webhook-fips image..."
  ...
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c 'command -v pipelines-webhook >/dev/null 2>&1 || ls /nix/store/*/bin/pipelines-webhook >/dev/null 2>&1'

  echo "All tekton-pipelines-webhook-fips tests passed!"
''
