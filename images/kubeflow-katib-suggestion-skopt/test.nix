...
pkgs.writeShellScript "test-kubeflow-katib-suggestion-skopt" ''
  ...
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c 'command -v kubeflow-katib-suggestion-skopt >/dev/null 2>&1 || ls /nix/store/*/bin/kubeflow-katib-suggestion-skopt >/dev/null 2>&1' || true
