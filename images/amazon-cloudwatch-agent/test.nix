...
pkgs.writeShellScript "test-amazon-cloudwatch-agent" ''
  ...
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c 'command -v amazon-cloudwatch-agent >/dev/null 2>&1 || ls /nix/store/*/bin/amazon-cloudwatch-agent >/dev/null 2>&1'
