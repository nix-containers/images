...
pkgs.writeShellScript "test-httpd" ''
  ...
  out=$(docker run --rm --entrypoint httpd ${image.imageName}:test -v 2>&1)
  [ -n "$out" ]

  # Binary present in image
  echo "  Checking httpd is present..."
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c 'command -v httpd >/dev/null 2>&1 || ls /nix/store/*/bin/httpd >/dev/null 2>&1'
