...
pkgs.writeShellScript "test-tekton-pipelines-nop-fips" ''
  set -euo pipefail
  echo "Testing tekton-pipelines-nop-fips image..."
  ...
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c 'command -v pipelines-nop >/dev/null 2>&1 || ls /nix/store/*/bin/pipelines-nop >/dev/null 2>&1' || true

  echo "All tekton-pipelines-nop-fips tests passed!"
''
