...
pkgs.writeShellScript "test-tekton-resolvers" ''
  set -euo pipefail
  echo "Testing tekton-resolvers image..."
  ...
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c 'command -v tekton-resolvers >/dev/null 2>&1 || ls /nix/store/*/bin/tekton-resolvers >/dev/null 2>&1' || true

  echo "All tekton-resolvers tests passed!"
''
