...
pkgs.writeShellScript "test-jaeger-es-rollover" ''
  set -euo pipefail
  echo "Testing jaeger-es-rollover image..."

  # Smoke test: the image starts a shell and the expected binary is present.
  # (jaeger-es-rollover is a long-running server; we avoid guessing version/help flags and
  ...
  echo "All jaeger-es-rollover tests passed!"
''
