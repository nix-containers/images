...
pkgs.writeShellScript "test-kubeflow-pipelines-scheduledworkflow" ''
  ...
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c 'command -v pipelines-scheduledworkflow >/dev/null 2>&1 || ls /nix/store/*/bin/pipelines-scheduledworkflow >/dev/null 2>&1'
