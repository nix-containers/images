...
pkgs.writeShellScript "test-jaeger-collector" ''
  set -euo pipefail
  echo "Testing jaeger-collector image..."

  # Smoke test: the image starts a shell and the expected binary is present.
  # (jaeger-collector is a long-running server; we avoid guessing version/help flags and
  ...
  echo "All jaeger-collector tests passed!"
''
