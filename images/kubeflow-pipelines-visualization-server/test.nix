...
pkgs.writeShellScript "test-kubeflow-pipelines-visualization-server" ''
  ...
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c 'command -v pipelines-visualization-server >/dev/null 2>&1 || ls /nix/store/*/bin/pipelines-visualization-server >/dev/null 2>&1' || true
