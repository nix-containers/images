...
# loki-fips-3.6-promtail — built from the upstream release artifact (#618).
let
  version = "3.6.11";
  ...
  drv = pkgs.runCommand "loki-fips-3.6-promtail-3.6.11" { nativeBuildInputs = [ pkgs.gnutar pkgs.gzip pkgs.unzip ]; } ''
...
mkImage {
  ...
  labels = {
    ...
    "org.opencontainers.image.description" = "loki-fips-3.6-promtail (built from upstream release 3.6.11)";
  };
}
