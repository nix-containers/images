...
in mkImage {
  ...
  name = "jaeger-2-es-rollover";
  tag = "v${version}";
  entrypoint = [ "${jaeger-component}/bin/2-es-rollover" ];
  cmd = [];
  extraPkgs = with pkgs; [ cacert tzdata ];
  labels = {
    "org.opencontainers.image.title" = "jaeger 2 es rollover";
    "org.opencontainers.image.description" = "Jaeger jaeger-2-es-rollover";
