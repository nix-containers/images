...
pkgs.writeShellScript "test-newrelic-infrastructure-k8s" ''
  ...
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c 'command -v newrelic-infrastructure-k8s >/dev/null 2>&1 || ls /nix/store/*/bin/newrelic-infrastructure-k8s >/dev/null 2>&1'
