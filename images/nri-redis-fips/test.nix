...
pkgs.writeShellScript "test-nri-redis-fips" ''
  set -euo pipefail
  echo "Testing nri-redis-fips image..."
  ...
  echo "  Checking redis binary is present..."
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c 'ls /nix/store/*/bin/redis >/dev/null 2>&1 && echo ok' | grep -q ok

  # 2. Image shell smoke test
  echo "  Checking image shell works..."
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c 'ls -la / >/dev/null && echo ok' | grep -q ok

  echo "All nri-redis-fips tests passed!"
''
