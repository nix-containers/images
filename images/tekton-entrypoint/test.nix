...
pkgs.writeShellScript "test-tekton-entrypoint" ''
  set -euo pipefail
  echo "Testing tekton-entrypoint image..."
  ...
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c 'command -v tekton-entrypoint >/dev/null 2>&1 || ls /nix/store/*/bin/tekton-entrypoint >/dev/null 2>&1'

  echo "All tekton-entrypoint tests passed!"
''
