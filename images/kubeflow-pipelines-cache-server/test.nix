...
pkgs.writeShellScript "test-kubeflow-pipelines-cache-server" ''
  ...
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c 'command -v kubeflow-pipelines-cache-server >/dev/null 2>&1 || ls /nix/store/*/bin/kubeflow-pipelines-cache-server >/dev/null 2>&1'
