...
pkgs.writeShellScript "test-knative-eventing-ingress-fips" ''
  set -euo pipefail
  echo "Testing knative-eventing-ingress-fips image..."
  ...
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c \
    'command -v knative-eventing-ingress >/dev/null 2>&1 || ls /nix/store/*/bin/knative-eventing-ingress >/dev/null 2>&1' || true
  ...
  echo "All knative-eventing-ingress-fips tests passed!"
''
