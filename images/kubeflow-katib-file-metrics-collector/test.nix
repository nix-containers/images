...
pkgs.writeShellScript "test-kubeflow-katib-file-metrics-collector" ''
  ...
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c 'command -v kubeflow-katib-file-metrics-collector >/dev/null 2>&1 || ls /nix/store/*/bin/kubeflow-katib-file-metrics-collector >/dev/null 2>&1' || true
