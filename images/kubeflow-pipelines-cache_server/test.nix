...
pkgs.writeShellScript "test-kubeflow-pipelines-cache_server" ''
  ...
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c 'command -v pipelines-cache_server >/dev/null 2>&1 || ls /nix/store/*/bin/pipelines-cache_server >/dev/null 2>&1' || true
