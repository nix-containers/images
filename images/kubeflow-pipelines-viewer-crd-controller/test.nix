...
pkgs.writeShellScript "test-kubeflow-pipelines-viewer-crd-controller" ''
  ...
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c 'command -v kubeflow-pipelines-viewer-crd-controller >/dev/null 2>&1 || ls /nix/store/*/bin/kubeflow-pipelines-viewer-crd-controller >/dev/null 2>&1' || true
