...
let
  ...
  component = buildGoModule {
    pname = "prometheus-mysqld-exporter-nixchart-fips";
    inherit version;
    src = fetchFromGitHub {
      owner = "prometheus";
      repo = "prometheus";
...
in mkImage {
  drv = component;
  name = "prometheus-mysqld-exporter-nixchart-fips";
  tag = "v${version}";
  entrypoint = [ "${component}/bin/prometheus-mysqld-exporter-iamguarded" ];
  cmd = [];
  extraPkgs = with pkgs; [ cacert tzdata ];
  labels = {
    "org.opencontainers.image.title" = "prometheus-mysqld-exporter-nixchart-fips";
