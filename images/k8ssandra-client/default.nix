...
# k8ssandra-client — built from the upstream release artifact (#618).
let
  version = "0.8.14";
  ...
  drv = pkgs.runCommand "k8ssandra-client-0.8.14" { nativeBuildInputs = [ pkgs.gnutar pkgs.gzip pkgs.unzip ]; } ''
...
mkImage {
  ...
  labels = {
    ...
    "org.opencontainers.image.description" = "k8ssandra-client (built from upstream release v0.8.14)";
  };
}
