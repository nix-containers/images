...
pkgs.writeShellScript "test-kubeflow-pipelines-apiserver" ''
  ...
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c 'command -v pipelines-apiserver >/dev/null 2>&1 || ls /nix/store/*/bin/pipelines-apiserver >/dev/null 2>&1' || true
