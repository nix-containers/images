...
pkgs.writeShellScript "test-kubeflow-pipelines-persistenceagent" ''
  ...
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c 'command -v kubeflow-pipelines-persistenceagent >/dev/null 2>&1 || ls /nix/store/*/bin/kubeflow-pipelines-persistenceagent >/dev/null 2>&1' || true
