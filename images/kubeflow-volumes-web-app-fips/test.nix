...
pkgs.writeShellScript "test-kubeflow-volumes-web-app-fips" ''
  ...
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c 'command -v kubeflow-volumes-web-app >/dev/null 2>&1 || ls /nix/store/*/bin/kubeflow-volumes-web-app >/dev/null 2>&1'
