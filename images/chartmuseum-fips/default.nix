{ mkImage, pkgs, lib, ... }:
# chartmuseum-fips - https://github.com/helm/chartmuseum
# Built from source + GOTOOLCHAIN=local to clear stale Go-stdlib CVEs.
let
  version = "0.16.5";
  drv = pkgs.buildGoModule {
    pname = "chartmuseum-fips"; inherit version;
...
in mkImage { inherit drv; name = "chartmuseum-fips"; tag = "v${version}"; entrypoint = [ "${drv}/bin/chartmuseum" ]; cmd = [ "--help" ]; labels = { "org.opencontainers.image.title" = "chartmuseum-fips"; "org.opencontainers.image.version" = version; "io.nix-containers.source" = "upstream-source"; }; }
