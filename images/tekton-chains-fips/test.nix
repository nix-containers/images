...
pkgs.writeShellScript "test-tekton-chains-fips" ''
  set -euo pipefail
  echo "Testing tekton-chains-fips image..."
  ...
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c 'command -v chains >/dev/null 2>&1 || ls /nix/store/*/bin/chains >/dev/null 2>&1' || true

  echo "All tekton-chains-fips tests passed!"
''
