{ mkImage, pkgs, lib, ... }:
# regclient-regbot (regclient) - https://github.com/regclient/regclient
# Built from source + GOTOOLCHAIN=local to clear stale Go-stdlib CVEs.
let
  version = "0.11.5";
  drv = pkgs.buildGoModule {
    pname = "regclient-regbot"; inherit version;
...
in mkImage { inherit drv; name = "regclient-regbot"; tag = "v${version}"; entrypoint = [ "${drv}/bin/regbot" ]; cmd = [ "--help" ]; labels = { "org.opencontainers.image.title" = "regclient-regbot"; "org.opencontainers.image.version" = version; "io.nix-containers.source" = "upstream-source"; }; }
