...
pkgs.writeShellScript "test-kubeflow-katib-controller" ''
  ...
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c 'command -v kubeflow-katib-controller >/dev/null 2>&1 || ls /nix/store/*/bin/kubeflow-katib-controller >/dev/null 2>&1' || true
