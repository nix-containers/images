...
# strimzi-kafka
# =============
# Apache Kafka (as shipped by Strimzi — Strimzi runs vanilla upstream Kafka).
# https://strimzi.io/  https://kafka.apache.org/
#
...
# sibling `kafka` image: a single-node KRaft broker+controller. In a real
# Strimzi deployment the Cluster Operator supplies its own config; this
# standalone broker is what the kind-test runs.
...
mkImage {
  ...
  labels = {
    ...
    "org.opencontainers.image.description" = "Apache Kafka (Strimzi distribution) - distributed event streaming platform";
