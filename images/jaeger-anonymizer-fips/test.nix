...
pkgs.writeShellScript "test-jaeger-anonymizer-fips" ''
  set -euo pipefail
  echo "Testing jaeger-anonymizer-fips image..."

  # Smoke test: the image starts a shell and the expected binary is present.
  # (jaeger-anonymizer-fips is a long-running server; we avoid guessing version/help flags and
  ...
  echo "All jaeger-anonymizer-fips tests passed!"
''
