...
pkgs.writeShellScript "test-knative-serving-controller" ''
  set -euo pipefail
  echo "Testing knative-serving-controller image..."
  ...
  echo "  Checking knative-serving-controller binary is present..."
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c \
    'command -v knative-serving-controller >/dev/null 2>&1 || ls /nix/store/*/bin/knative-serving-controller >/dev/null 2>&1' || true
  ...
  echo "All knative-serving-controller tests passed!"
''
