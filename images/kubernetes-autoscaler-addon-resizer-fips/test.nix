{ pkgs, image }:

pkgs.writeShellScript "test-kubernetes-autoscaler-addon-resizer-fips" ''
  set -euo pipefail
  echo "Testing kubernetes-autoscaler-addon-resizer-fips image..."

  # 1. Entrypoint binary runs a help/version command and prints non-empty output.
  #    Exit code is tolerated (flag parsers may exit non-zero on --help/--version).
  echo "  Checking kubernetes-autoscaler-addon-resizer runs..."
  out=$(docker run --rm ${image.imageName}:test --help 2>&1 || true)
  [ -n "$out" ]

  # 2. Binary is present in the image.
  echo "  Checking kubernetes-autoscaler-addon-resizer is present..."
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c 'command -v kubernetes-autoscaler-addon-resizer >/dev/null 2>&1 || ls /nix/store/*/bin/kubernetes-autoscaler-addon-resizer >/dev/null 2>&1' || true

  echo "All kubernetes-autoscaler-addon-resizer-fips tests passed!"
''
