...
# kafka-proxy — built from the upstream release artifact (#618).
let
  version = "0.4.3";
  ...
  drv = pkgs.runCommand "kafka-proxy-0.4.3" { nativeBuildInputs = [ pkgs.gnutar pkgs.gzip pkgs.unzip ]; } ''
...
mkImage {
  ...
  labels = {
    ...
    "org.opencontainers.image.description" = "kafka-proxy (built from upstream release 0.4.3)";
  };
}
