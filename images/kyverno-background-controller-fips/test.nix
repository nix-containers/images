...
pkgs.writeShellScript "test-kyverno-background-controller-fips" ''
  ...
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c 'command -v background-controller >/dev/null 2>&1 || ls /nix/store/*/bin/background-controller >/dev/null 2>&1'
