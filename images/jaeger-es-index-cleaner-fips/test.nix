...
pkgs.writeShellScript "test-jaeger-es-index-cleaner-fips" ''
  set -euo pipefail
  echo "Testing jaeger-es-index-cleaner-fips image..."

  # Smoke test: the image starts a shell and the expected binary is present.
  # (jaeger-es-index-cleaner-fips is a long-running server; we avoid guessing version/help flags and
  ...
  echo "All jaeger-es-index-cleaner-fips tests passed!"
''
