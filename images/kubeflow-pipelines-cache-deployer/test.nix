...
pkgs.writeShellScript "test-kubeflow-pipelines-cache-deployer" ''
  ...
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c 'command -v pipelines-cache-deployer >/dev/null 2>&1 || ls /nix/store/*/bin/pipelines-cache-deployer >/dev/null 2>&1'
