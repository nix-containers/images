...
pkgs.writeShellScript "test-kubeflow-pipelines-metadata-writer" ''
  ...
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c 'command -v pipelines-metadata-writer >/dev/null 2>&1 || ls /nix/store/*/bin/pipelines-metadata-writer >/dev/null 2>&1' || true
