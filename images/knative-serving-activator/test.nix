...
pkgs.writeShellScript "test-knative-serving-activator" ''
  set -euo pipefail
  echo "Testing knative-serving-activator image..."
  ...
  echo "  Checking knative-serving-activator binary is present..."
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c \
    'command -v knative-serving-activator >/dev/null 2>&1 || ls /nix/store/*/bin/knative-serving-activator >/dev/null 2>&1' || true
  ...
  echo "All knative-serving-activator tests passed!"
''
