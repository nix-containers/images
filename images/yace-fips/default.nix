...
# yace-fips — built from the upstream release artifact (#618).
let
  version = "0.67.0";
  ...
  drv = pkgs.runCommand "yace-fips-0.67.0" { nativeBuildInputs = [ pkgs.gnutar pkgs.gzip pkgs.unzip ]; } ''
...
mkImage {
  ...
  labels = {
    ...
    "org.opencontainers.image.description" = "yace-fips (built from upstream release v0.67.0)";
  };
}
