...
pkgs.writeShellScript "test-nri-nagios-fips" ''
  set -euo pipefail
  echo "Testing nri-nagios-fips image..."
  ...
  echo "  Checking nagios binary is present..."
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c 'ls /nix/store/*/bin/nagios >/dev/null 2>&1 && echo ok' | grep -q ok

  # 2. Image shell smoke test
  echo "  Checking image shell works..."
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c 'ls -la / >/dev/null && echo ok' | grep -q ok

  echo "All nri-nagios-fips tests passed!"
''
