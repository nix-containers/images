...
pkgs.writeShellScript "test-nri-f5-fips" ''
  set -euo pipefail
  echo "Testing nri-f5-fips image..."
  ...
  echo "  Checking f5 binary is present..."
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c 'ls /nix/store/*/bin/f5 >/dev/null 2>&1 && echo ok' | grep -q ok

  # 2. Image shell smoke test
  echo "  Checking image shell works..."
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c 'ls -la / >/dev/null && echo ok' | grep -q ok

  echo "All nri-f5-fips tests passed!"
''
