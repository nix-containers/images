...
let
  ...
  component = buildGoModule {
    pname = "prometheus-alertmanager-nixchart-fips";
    inherit version;
    src = fetchFromGitHub {
      owner = "prometheus";
      repo = "prometheus";
...
in mkImage {
  drv = component;
  name = "prometheus-alertmanager-nixchart-fips";
  tag = "v${version}";
  entrypoint = [ "${component}/bin/prometheus-alertmanager-iamguarded" ];
  cmd = [];
  extraPkgs = with pkgs; [ cacert tzdata ];
  labels = {
    "org.opencontainers.image.title" = "prometheus-alertmanager-nixchart-fips";
