...
pkgs.writeShellScript "test-knative-eventing-mtchannel-broker-fips" ''
  set -euo pipefail
  echo "Testing knative-eventing-mtchannel-broker-fips image..."
  ...
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c \
    'command -v knative-eventing-mtchannel-broker >/dev/null 2>&1 || ls /nix/store/*/bin/knative-eventing-mtchannel-broker >/dev/null 2>&1'

  echo "  Checking root filesystem is accessible..."
  out=$(docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c 'ls -la / && echo ok' 2>&1)
  echo "$out" | grep -q ok

  echo "All knative-eventing-mtchannel-broker-fips tests passed!"
''
