...
pkgs.writeShellScript "test-tekton-pipelines-resolvers-fips" ''
  ...
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c 'command -v pipelines-resolvers >/dev/null 2>&1 || ls /nix/store/*/bin/pipelines-resolvers >/dev/null 2>&1' || true
