...
pkgs.writeShellScript "test-dotnet-sdk" ''
  ...
  out=$(docker run --rm ${image.imageName}:test --info 2>&1)
  [ -n "$out" ]

  # 2. dotnet binary present in image (shell is bundled)
  echo "  Checking dotnet binary is present..."
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c 'command -v dotnet >/dev/null 2>&1 || ls /nix/store/*/bin/dotnet >/dev/null 2>&1'
