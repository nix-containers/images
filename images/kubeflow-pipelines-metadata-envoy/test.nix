...
pkgs.writeShellScript "test-kubeflow-pipelines-metadata-envoy" ''
  ...
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c 'command -v kubeflow-pipelines-metadata-envoy >/dev/null 2>&1 || ls /nix/store/*/bin/kubeflow-pipelines-metadata-envoy >/dev/null 2>&1'
