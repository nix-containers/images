...
pkgs.writeShellScript "test-kube-scheduler-1.34-default" ''
  set -euo pipefail
  echo "Testing kube-scheduler-1.34-default image..."
  ...
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c \
    'command -v scheduler >/dev/null 2>&1 || ls /nix/store/*/bin/scheduler >/dev/null 2>&1' || true

  echo "All kube-scheduler-1.34-default tests passed!"
''
