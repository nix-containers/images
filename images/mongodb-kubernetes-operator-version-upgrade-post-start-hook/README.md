...
```bash
# Build the image
nix build .#mongodb-kubernetes-operator-version-upgrade-post-start-hook

# Load into Docker
nix build .#load-mongodb-kubernetes-operator-version-upgrade-post-start-hook-to-docker && ./result/bin/load-mongodb-kubernetes-operator-version-upgrade-post-start-hook-to-docker
```
