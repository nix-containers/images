...
pkgs.writeShellScript "test-kubeflow-katib-suggestion-optuna" ''
  ...
  docker run --rm --entrypoint /bin/sh ${image.imageName}:test -c 'command -v kubeflow-katib-suggestion-optuna >/dev/null 2>&1 || ls /nix/store/*/bin/kubeflow-katib-suggestion-optuna >/dev/null 2>&1' || true
